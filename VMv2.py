...
def gettheCTE(value):
    global CONSTANTSvar_set
    for key, value2 in CONSTANTSvar_set.items():
...
def loadvirtualmemory(processname): # LOAD THE MEMORY WE ARE WORKING WITH
    for element in TABLEof_functions[processname]['variables']:
        virtualaddr = TABLEof_functions[processname]['variables'][element]['virtualaddress'] # GET THE VIRTUAL ADDRESS
...
for x in TABLEof_functions.keys(): # GET THE THE NAME OF THE FUNCTION PROGRAM
...
for element in TABLEof_functions[actualprogramname]['variables']: # LOAD THE GLOBAL MEMORY
    virtualaddr = TABLEof_functions[actualprogramname]['variables'][element]['virtualaddress']
    GLOBALmemory.memor[virtualaddr] = None

for element2 in CONSTANTSvar_set: # LOAD THE CONSTANT TO THE MEMORY
    virtualaddr = CONSTANTSvar_set[element2]
